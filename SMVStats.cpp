...
public:
    ...
    __always_inline void constructor(address SMV_root) {
        SMV_root_ = SMV_root;
        transfers_.reset();
        start_balance_ = tvm_balance();
    }

    __always_inline bool_t registerTransfer(uint256 proposalId, address contestAddr, uint256 requestValue) {
    ...
    // ======= getters =======
    __always_inline dict_array<Transfer> getTransfers() {
        dict_array<Transfer> rv;
        for (auto it = transfers_.rbegin(); it != transfers_.rend(); ++it) {
            auto rec = *it;
            rv.push_back(Transfer {rec.proposalId,
