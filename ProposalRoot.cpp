...
public:
    ...
    __always_inline void deployProposal() {
    ...
    __always_inline VotesInfo vote(uint256 pubkey, DepositType deposit, bool_t yes) {
    ...
    __always_inline bool_t checkFinished() {
        set_int_return_flag(SEND_REST_GAS_FROM_INCOMING);
        if (isFinished()) {
    ...
    // Also generates external message with voting results at first request after voting finish
    __always_inline bool_t checkApproved() {
        set_int_return_flag(SEND_REST_GAS_FROM_INCOMING);
    ...
    // Also generates external message with voting results at first request after voting finish
    __always_inline VoteResults checkResults() {
        set_int_return_flag(SEND_REST_GAS_FROM_INCOMING);
    ...
    __always_inline void contestApproved() {
    ...
    __always_inline bool_t getSuperMajority() {
    ...
    __always_inline DepositType getVotePrice() {
        return votePrice_;
    }

    __always_inline cell getBallotCode() {
        return ballot_code_;
    }

    __always_inline ProposalInfo getProposal() {
        return ProposalInfo {id_,
                             start_,
                             end_,
                             desc_,
                             isFinished(),
    ...
    __always_inline bool_t isFinished() {
        return bool_t {isEarlyFinished() || (smart_contract_info::now() > end_)};
    }

    __always_inline bool_t isApproved() {
        if (!isFinished())
            return bool_t {false};
    ...
    __always_inline bool_t isResultSent() {
        return result_sent_;
    }

    __always_inline bool_t isEarlyFinished() {
        return bool_t {checkSureApproved() || checkSureDisapproved()};
    }

    __always_inline bool_t isWhiteListEnabled() {
        return white_list_enabled_;
    }

    __always_inline dict_array<uint256> getWhiteList() {
...
private:
    __always_inline uint256 expected_sender_address(uint256 pubkey) const {
        DMultiBallot ballot_data {pubkey, workchain_id_, super_root_, depool_, DepositType(0), DepositType(0), {}};
        return prepare_ballot_state_init_and_addr(ballot_data, ballot_code_).second;
    }

    __always_inline void sendResultsImpl(bool_t approve) {
        if (!result_sent_) {
    ...
    __always_inline bool checkSoftSimpleMajority() const {
        // Ymax = T/2 + 1; Nmax = T/2
        // Ymin = T/10;    Nmin = 0
        // Y = Ymin + k * N
        // T/2 + 1 = T/10 + k * (T/2)
    ...
    __always_inline bool checkSoftSuperMajority() const {
        // Ymax = T/2 + 1; Nmax = T/2
        // Ymin = T/3;     Nmin = 0
        // Y = Ymin + k * N
        // T/2 + 1 = T/3 + k * (T/2)
    ...
    __always_inline bool checkSureApproved() const {
        return super_majority_ ? checkSureApprovedSuper() : checkSureApprovedSimple();
    }
    __always_inline bool checkSureDisapproved() const {
        return super_majority_ ? checkSureDisapprovedSuper() : checkSureDisapprovedSimple();
    }
    __always_inline bool checkSureApprovedSimple() const {
        auto T = totalVotes_.get();
        auto Y = yesVotes_.get();
        return 2 * Y > T;
    }
    __always_inline bool checkSureDisapprovedSimple() const {
        auto T = totalVotes_.get();
        auto N = noVotes_.get();
        return 2 * N > T;
    }
    __always_inline bool checkSureApprovedSuper() const {
        auto T = totalVotes_.get();
        auto Y = yesVotes_.get();
        return 3 * Y > 2 * T;
    }
    __always_inline bool checkSureDisapprovedSuper() const {
        auto T = totalVotes_.get();
        auto N = noVotes_.get();
        return 3 * N > 2 * T;
    }
    ...
    __always_inline cell convertExternalDeployIntoInternal(cell orig_msg, Grams value) {
        ...
        int_msg_info_relaxed out_info = {{},
                                         /*ihr_disabled*/ bool_t {true},
                                         /*bounce*/ bool_t {false},
                                         bool_t {false},
                                         MsgAddress {MsgAddressExt {addr_none {}}},
                                         addr,
                                         {value, {}},
                                         Grams(0),
                                         Grams(0),
