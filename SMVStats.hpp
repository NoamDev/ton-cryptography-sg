...
namespace tvm {
    namespace schema {
        ...
        __interface [[no_pubkey]] ISMVStats {
            ...
            [[internal, noaccept, answer_id]] bool_t registerTransfer(uint256 proposalId, address contestAddr,
                                                                      uint256 requestValue);

            // ============== getters ==============
            [[getter]] dict_array<Transfer> getTransfers();
        };
        ...
        inline std::pair<StateInit, uint256> prepare_stats_state_init_and_addr(DSMVStats data, cell code) {
            cell data_cl = prepare_persistent_data<ISMVStats, stats_replay_protection_t, DSMVStats>(
                {stats_replay_protection_t::init()}, data);
            StateInit init {/*split_depth*/ {}, /*special*/ {}, code, data_cl, /*library*/ {}};
