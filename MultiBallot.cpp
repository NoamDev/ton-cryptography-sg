...
public:
    ...
    __always_inline void deployBallot() {
    ...
    __always_inline void receiveNativeTransfer(DepositType amount) {
    ...
    __always_inline void receiveStakeTransfer(address source, uint128 amount) {
    ...
    __always_inline sendVotesResult sendVote(address proposal, bool_t yesOrNo) {
    ...
    __always_inline resumable<void> requestDeposit(address user_wallet) {
        require(tvm_pubkey() == ballot_public_key_, error_code::message_sender_is_not_my_owner);
        ...
        for (auto proposal : proposals_) {
            ...
            bool_t finished = co_await proposal_root(Grams(isFinishedTons)).checkFinished();
            if (!finished)
    ...
    __always_inline void finalize(address user_wallet) {
        require(tvm_pubkey() == ballot_public_key_, error_code::message_sender_is_not_my_owner);
    ...
    __always_inline DepositType getNativeDeposit() {
        return native_deposit_;
    }

    __always_inline DepositType getStakeDeposit() {
        return stake_deposit_;
    }

    __always_inline dict_array<uint256> getProposals() {
    ...
    __always_inline dict_array<DepositType> getProposalVoteDeposits() {
    ...
    __always_inline DepositType getProposalRoot_Vote_GasPrice() {
        return DepositType(gastogram(ProposalRoot_Vote_Gas));
    }

    __always_inline DepositType getProposalRoot_ReturnDeposit_GasPrice() {
        return DepositType(gastogram(ProposalRoot_ReturnDeposit_Gas));
    }

    __always_inline DepositType getMultiBallot_receiveNativeTransfer_GasPrice() {
        return DepositType(gastogram(MultiBallot_receiveNativeTransfer_Gas));
    }

    __always_inline DepositType getMultiBallot_receiveStakeTransfer_GasPrice() {
