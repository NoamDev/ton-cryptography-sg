...
#define GASTOGRAM_WORKAROUND
        ...
        __interface IMultiBallot {
            ...
            [[internal, noaccept]] void deployBallot();

            // Receive native funds transfer and keep it in deposit
            [[internal, noaccept]] void receiveNativeTransfer(DepositType amount);

            // Receive stake transfer notify (from solidity IParticipant::onTransfer(address source, uint128 amount))
            [[internal, noaccept]] void receiveStakeTransfer(address source, uint128 amount) =
                0x23c4771d;    // = hash_v<"onTransfer(address,uint128)()v2">

            // ======= externals =======

            // Sends all votes (for deposit) to Proposal Root contract
            [[external, noaccept, dyn_chain_parse]] sendVotesResult sendVote(address proposal, bool_t yesOrNo);
            ...
            [[external, noaccept, dyn_chain_parse]] resumable<void> requestDeposit(address user_wallet);

            // send all remaining tons to user_wallet
            [[external, noaccept, dyn_chain_parse]] void finalize(address user_wallet);

            // ======= getters =======

            [[getter]] address getDepool();

            [[getter]] DepositType getNativeDeposit();

            [[getter]] DepositType getStakeDeposit();

            [[getter, no_persistent]] DepositType getMultiBallot_receiveNativeTransfer_GasPrice();

            [[getter, no_persistent]] DepositType getMultiBallot_receiveStakeTransfer_GasPrice();
        };
        ...
        inline std::pair<StateInit, uint256> prepare_ballot_state_init_and_addr(DMultiBallot ballot_data,
                                                                                cell ballot_code) {
            cell ballot_data_cl = prepare_persistent_data<IMultiBallot, multi_ballot_replay_protection_t, DMultiBallot>(
                {multi_ballot_replay_protection_t::init(), {}}, ballot_data);
            StateInit ballot_init {/*split_depth*/ {}, /*special*/ {}, ballot_code, ballot_data_cl, /*library*/ {}};
