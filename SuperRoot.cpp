...
public:
    ...
    __always_inline void constructor(address budget, address stats, address depool) {
        budget_ = budget;
        stats_ = stats;
        depool_ = depool;
    ...
    __always_inline void setProposalRootCode(cell code) {
        require(deployer_key_ && deployer_key_ == tvm_pubkey(), error_code::wrong_deployer_key);
        tvm_accept();
        proposal_root_code_ = code;
        // when the contract is fully initialized, deployer key will be
        // nullified
        if (proposal_root_code_ && multi_ballot_code_)
            deployer_key_ = 0;
    }

    __always_inline void setMultiBallotCode(cell code) {
        require(deployer_key_ && deployer_key_ == tvm_pubkey(), error_code::wrong_deployer_key);
        tvm_accept();
    ...
    // Adds new proposal to m_proposals, prepares initial data for new Proposal
    // Root contract,
    //  calculates it address and deploys it
    __always_inline bool_t createProposal(uint256 id, VotesType totalVotes, uint32 startime, uint32 endtime, bytes desc,
                                          bool_t super_majority, DepositType vote_price, bool_t finalMsgEnabled,
        ...
        auto [proposal_init, dest] =
            calc_proposal_init(id, totalVotes, startime, endtime, desc, super_majority, vote_price, whiteListEnabled,
                               whitePubkeys, finalMsgOpt, finalMsgValue, finalMsgRequestValue);
    ...
    __always_inline address createMultiBallot(uint256 pubkey, DepositType tonsToBallot) {
        require(proposal_root_code_ && multi_ballot_code_, error_code::not_fully_initialized);
        ...
        dest_handle.deploy(ballot_init, Grams(tonsToBallot.get()), IGNORE_ACTION_ERRORS).deployBallot();
        set_int_return_flag(INT_RETURN_FLAG);
        return dest;
    }

    __always_inline void contestApproved(uint256 id, address contest_addr, uint256 requestValue) {
        require(proposal_root_code_ && multi_ballot_code_, error_code::not_fully_initialized);
        ...
        budget_(Grams(0), SEND_REST_GAS_FROM_INCOMING).request(id, contest_addr, requestValue);
    }

    // ============== getters ==============
    __always_inline address getBudget() {
        return budget_.get();
    }

    __always_inline address getStats() {
        return stats_.get();
    }

    __always_inline address getDepool() {
        return depool_;
    }

    __always_inline address getMultiBallotAddress(uint256 pubkey) {
        return calc_ballot_init(pubkey).second;
    }

    // Returns code of Proposal Root smart contract
    __always_inline cell getProposalRootCode() {
        return *proposal_root_code_;
    }

    // Returns code of Voting Wallet smart contract
    __always_inline cell getMultiBallotCode() {
        return *multi_ballot_code_;
    }

    __always_inline int8 getWorkchainId() {
        return workchain_id_;
    }

    __always_inline DepositType getStartBalance() {
        return start_balance_;
    }

    __always_inline Proposal getProposalById(uint256 id) {
        require(proposals_.contains(id.get()), error_code::id_not_found);
        return proposals_.get_at(id.get());
    }

    __always_inline dict_array<uint256> getProposalIds() {
    ...
    __always_inline address getProposalAddress(uint256 id) {
        return getProposalById(id).root;
    }

    __always_inline bool_t isFullyInitialized() {
        return bool_t {proposal_root_code_ && multi_ballot_code_};
    }

    __always_inline uint32 getNow() {
        return uint32(smart_contract_info::now());
    }

    __always_inline DepositType getCreateProposalGasPrice() {
        return DepositType(gastogram(createProposalGas));
    }

    __always_inline DepositType getDeployProposalGasPrice() {
        return DepositType(gastogram(deployProposalGas));
    }

    __always_inline DepositType getCreateMultiBallotGasPrice() {
        return DepositType(gastogram(createMultiBallotGas));
    }

private:
    __always_inline std::pair<StateInit, address>
        calc_proposal_init(uint256 id, VotesType totalVotes, uint32 startime, uint32 endtime, bytes desc,
                           bool_t super_majority, DepositType vote_price, bool_t whiteListEnabled,
                           const dict_array<uint256> whitePubkeys, std::optional<cell> finalMsg, uint256 finalMsgValue,
        ...
        DProposalRoot proposal_data {super_majority,
                                     workchain_id_,
                                     my_std_addr,
                                     depool_,
                                     vote_price,
                                     *multi_ballot_code_,
                                     finalMsg,
                                     Grams(finalMsgValue.get()),
                                     Grams(finalMsgRequestValue.get()),
                                     id,
                                     startime,
                                     endtime,
                                     desc,
    ...
    __always_inline std::pair<StateInit, address> calc_ballot_init(uint256 pubkey) const {
        auto myaddr = address {tvm_myaddr()};
        auto my_std_addr = std::get<addr_std>(myaddr()).address;
        DMultiBallot ballot_data {pubkey, workchain_id_, my_std_addr, depool_, DepositType {0}, DepositType {0}, {}};
