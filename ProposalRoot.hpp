...
namespace tvm {
    namespace schema {
        ...
        __interface IProposalRoot {
            ...
            [[internal, noaccept]] void deployProposal() = 11;

            // Allows a multi ballot to vote for proposal
            [[internal, noaccept, answer_id]] VotesInfo vote(uint256 pubkey, DepositType deposit, bool_t yes) = 12;
            ...
            [[internal, noaccept, answer_id]] bool_t checkFinished() = 13;

            // Returns true if voting is already finished and approved
            // Also generates external message with voting results at first request after voting finish
            [[internal, noaccept, answer_id]] bool_t checkApproved() = 14;

            // Returns VoteResults with details
            // Also generates external message with voting results at first request after voting finish
            [[internal, noaccept, answer_id]] VoteResults checkResults() = 15;

            // contest notifies that it is approved
            [[internal, noaccept]] void contestApproved() = 16;

            // ========== getters ==========
            [[getter]] address getDepool();

            // Returns a price value in tons that must be deposited for one vote
            [[getter]] DepositType getVotePrice();

            // Returns full information about proposal
            [[getter]] ProposalInfo getProposal();

            [[getter]] dict_array<uint256> getWhiteList();
        };
        ...
        inline std::pair<StateInit, uint256> prepare_proposal_state_init_and_addr(DProposalRoot proposal_data,
                                                                                  cell proposal_code) {
            ...
            StateInit proposal_init {/*split_depth*/ {}, /*special*/ {}, proposal_code, proposal_data_cl,
                                     /*library*/ {}};
